...
class ProcureXApp {
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            const demoLauncher = e.target.closest('[data-demo-launch]');
            if (demoLauncher) {
                e.preventDefault();
                this.launchDemo();
                return;
            ...
            const notificationClose = e.target.closest('[data-notification-close]');
            if (notificationClose) {
                e.preventDefault();
                this.dismissNotification(notificationClose.getAttribute('data-notification-close'));
                return;
            }
    ...
    setupNotifications() {
        this.ensureNotificationRegion();
        window.alert = (message) => this.showNotification(message, { tone: 'warning' });
    }
    ...
    ensureNotificationRegion() {
        ...
        region.setAttribute('data-notification-region', '');
    ...
    showNotification(message, options = {}) {
        const region = this.ensureNotificationRegion();
        const id = `notification-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`;
        const tone = options.tone || 'info';
        const notification = document.createElement('article');
        notification.className = `notification-card notification-card-${tone}`;
        notification.setAttribute('data-notification-id', id);
        notification.setAttribute('role', tone === 'error' || tone === 'warning' ? 'alert' : 'status');

        const copy = document.createElement('div');
        copy.className = 'notification-card-copy';
        const title = document.createElement('strong');
        title.textContent = options.title || (tone === 'error' ? 'Action needed' : tone === 'success' ? 'Success' : 'Notice');
        const body = document.createElement('span');
        body.textContent = String(message || 'Done.');
        copy.append(title, body);

        const closeButton = document.createElement('button');
        closeButton.className = 'notification-card-close';
        closeButton.type = 'button';
        closeButton.setAttribute('aria-label', 'Close notification');
        closeButton.setAttribute('data-notification-close', id);
        ...
        const timer = window.setTimeout(() => this.dismissNotification(id), options.duration || 5200);
    ...
    dismissNotification(id) {
        if (!id) return;
        const notification = document.querySelector(`[data-notification-id="${CSS.escape(id)}"]`);
    ...
    handleSignIn(form) {
        ...
        this.applyMockAuthSession(account, email);
    ...
    getDemoAuthAccount() {
        const accounts = mockData.mockAuth?.accounts || [];
        return this.findMockAuthAccount('demo@procurex.tz')
            || accounts.find(account => account.accountType !== 'admin' && account.ekycCompleted)
            || accounts.find(account => account.accountType !== 'admin')
            || accounts[0]
            || null;
    }

    applyMockAuthSession(account, email = account?.email || '') {
        if (!account) return;
        ...
        mockData.users.current = {
            ...
            organization: account.organization || account.displayName || email,
            trustTier: account.trustTier || (account.ekycCompleted ? 'Verified' : 'Pending'),
        ...
        mockData.users.supplier = {
            ...
            name: account.displayName || account.organization || email,
            organization: account.organization || account.displayName || email,
            trustTier: account.trustTier || mockData.users.supplier?.trustTier
        };
        mockData.users.buyer = {
            ...(mockData.users.buyer || {}),
            name: account.displayName || account.organization || email,
            organization: account.organization || account.displayName || email,
            trustTier: account.trustTier || mockData.users.buyer?.trustTier
        };
    }
